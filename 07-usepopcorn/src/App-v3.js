...
import { useMoives } from './useMovie';
...
export default function App() {
  ...
  const { movies, isLoading, error } = useMoives(query);
...
function Search({ query, setQuery }) {
  ...
  return (
    <input
      ...
      // Step2: Go the the element that we want to select. And pass in the ref and put the Ref we just created
      ref={inputEl}
    />
  );
}
...
function MovieDetails({ selectedId, onCloseMovie, onAddWatched, watched }) {
  ...
  // The value of this hook will stay as false forever
          ...
          <section>
            {isWatched ? (
              <div className="rating">
                <span>You rated this moive {watchedUserRating}</span>
              </div>
            ) : (
              <div className="rating">
                <StarRating
